...
// Extends companyToolsPrefix() with a user segment, for tools that need
// per-user isolation inside a company's tools folder (Listing Tools product
// images: tools/{companyId}/{userId}/listing-tools/{templateId}/...) —
// uploadCompanyMedia() alone only goes one level deep (company → subFolder).
export async function uploadUserToolMedia(file, folderId, userId, subFolder = 'general') {
  const folder = `${companyToolsPrefix(folderId)}/${userId}/${subFolder}`
  const { nanoid } = await import('nanoid')
  const ext = file.name.split('.').pop().toLowerCase()
  const pathname = `${folder}/${nanoid()}.${ext}`
  const blob = await put(pathname, file, {
