...
// Template-level audit entry (create / edit / AI rules / visibility /
// delete) in the hub's listing_template_logs — the table the Template Logs
// page and each template's Versions tab read, distinct from the free-action
// history above. Same best-effort contract as recordTemplateHistory, except
// it hands back the batch id the row was stamped with (null on any
// failure), so a follow-up version save can join the same batch. `log` is
// the { action, detail } lib/templateLogDiff.js builds.
export async function recordTemplateLog(req, templateId, log) {
  if (!ADMIN_URL || !log) return null
  try {
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), 4000)
    const res = await fetch(`${ADMIN_URL}/api/listing-tools/logs`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(authHeaderFrom(req) ? { Authorization: authHeaderFrom(req) } : {}),
      },
      body: JSON.stringify({ templateId, action: log.action, detail: log.detail }),
      signal: controller.signal,
    }).finally(() => clearTimeout(timeout))
    const data = await res.json().catch(() => null)
    return data?.log?.batchId ?? null
  } catch {
    return null
  }
}
...
// Delete counterpart to syncProductDetailsHistory/syncPrefillDetailsHistory above — the hub's
// upsert has no way to express "this row is gone" (it only ever adds-or-updates whatever it's
// given), so a product/brand that disappears from a save — deleted outright, or its key field
// edited to a new value — would otherwise leave a stale record in the hub forever, still
// resurfacing via Product Group auto-fill and the History page. Fired from sheets/[group]/
// route.js's PATCH handler whenever it finds a key present before the write but missing after.
// Same best-effort, fire-and-forget contract as the sync functions above.
async function deleteHistoryRow(req, path, params) {
  if (!ADMIN_URL) return
  try {
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), 4000)
    await fetch(`${ADMIN_URL}${path}?${new URLSearchParams(params)}`, {
      method: 'DELETE',
      headers: { ...(authHeaderFrom(req) ? { Authorization: authHeaderFrom(req) } : {}) },
      signal: controller.signal,
    }).finally(() => clearTimeout(timeout))
  } catch {
    // best-effort only — never surfaces to the caller
...
export async function deleteProductDetailsHistory(req, { templateId, productNumber }) {
  await deleteHistoryRow(req, '/api/listing-tools/product-details-history', { templateId, productNumber })
}

export async function deletePrefillDetailsHistory(req, { templateId, brand }) {
  await deleteHistoryRow(req, '/api/listing-tools/prefill-details-history', { templateId, brand })
}
