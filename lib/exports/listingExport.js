...
// Fallback only for a sheet with no stored `sheetName` at all (pre-dates that field existing) —
// "optional" stays mapped here (even though it's a retired group, see
// TemplateSettingsWizard.jsx's own GROUPS comment) purely so an old export of a template that
// still has a leftover empty Optional sheet object never shows a raw group id as a tab/heading.
const GROUP_LABELS = {
...
export async function downloadExcel(template, { groups, filename } = {}) {
  ...
  for (const sheet of sheetsForGroups(template, groups || ['design_system', 'compulsory', 'prefill'])) {
...
export async function downloadPdf(template, { groups, filename } = {}) {
  ...
  const sheets = sheetsForGroups(template, groups || ['design_system', 'compulsory', 'prefill'])
...
export async function downloadExcelSmart(template, meta, { groups, filename } = {}) {
  const effectiveGroups = groups || ['design_system', 'compulsory', 'prefill']
