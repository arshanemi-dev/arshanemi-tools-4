...
function sheetToAoa(sheet) {
  ...
  const filledRows = (sheet.rows || []).filter((row) => Object.values(row).some((v) => String(v ?? '').trim()))
...
export async function downloadExcelSmart(template, meta, { groups, filename } = {}) {
  ...
  const groupSheets = sheetsForGroups(template, effectiveGroups).map((sheet) => ({
    ...
    // One row per Multi Select option, same as the plain export above —
    // only at export time, never in the on-screen grid.
    rows: expandMultiSelectRows(sheet.headers, sheet.rows),
