...
//
// A LARGE enum hits a different flavor of the same 400, and Google's error
// body gives zero indication which field or why ("Request contains an
// invalid argument", no field name). Live-bisected against this account: a
// 193-country "Country of Origin" dropdown fails at exactly 127 enum values
// (126 succeeds), while a 133-value "Bust Size" dropdown (short numeric
// strings) succeeds outright — so this is a serialized-size/token budget for
// one enum property, not a raw item-count cap, and it silently kills the
// ENTIRE row's request (every other field in the same call), not just the
// oversized one. `MAX_ENUM_SCHEMA_CHARS` is set well under the ~1400-1460
// char boundary observed at that break point to leave margin for
// longer-token content. Past the cap, the field drops to a free-text STRING
// with no schema-level `enum` — it still gets its allowed-values list in the
// prompt text (lib/aiFillPrompt.js's buildPrompt) and is still validated
// against `dropdownValues` afterward by sanitizeGeneratedFields, so this only
// trades the grammar-enforced guarantee for a prompt-and-verify one, it
// doesn't skip validation.
const MAX_ENUM_SCHEMA_CHARS = 1000
function enumFitsSchema(values) {
  return values.reduce((sum, v) => sum + v.length + 3, 0) <= MAX_ENUM_SCHEMA_CHARS
}

function buildResponseSchema(targets) {
  const properties = {}
  for (const t of targets) {
    const useEnum = t.dropdownValues?.length && enumFitsSchema(t.dropdownValues)
    properties[t.id] = {
      type: Type.STRING,
      description: t.label,
      ...(useEnum ? { format: 'enum', enum: t.dropdownValues } : {}),
    }
  }
