...
// header columns) and the master source workbook a template is created
// from. Trimmed to just what Listing Tools needs (upload + shared link +
// delete); no fuller file-browser ops like tools-1's lib/storage/dropbox.js
// (list/move/copy) since nothing here browses Dropbox — every upload is a
// one-off file, occasionally superseded by a replacement (hence deleteFile).
...
// Dropbox's error_summary is always a slash-joined tag path (e.g.
// "path/not_found/..."), which is a more reliable not-found check than
// walking the nested tagged-union shape by hand.
function isNotFoundError(err) {
  return err?.status === 409 && /^path\/not_found/.test(err?.error?.error_summary || '')
}

// Reads one JSON file back out of Dropbox — the read half of the
// key-value-JSON "database" pattern used by lib/listingTemplates.js in
// place of Vercel Blob (lib/blobStore.js). Returns `fallback` for a key
// that's never been written yet, same contract as blobStore.js's
// readBlobJson.
export async function readJsonFile(path, fallback = null) {
  const dbx = await getDbx()
  try {
    const res = await withRetry(() => dbx.filesDownload({ path }))
    return JSON.parse(res.result.fileBinary.toString('utf-8'))
  } catch (err) {
...
// Writes (creating or overwriting) one JSON file at an exact path — unlike
// uploadFile, this never autorenames, since callers need a stable path to
// read the same key back from.
export async function writeJsonFile(path, data) {
  const dbx = await getDbx()
  const buffer = Buffer.from(JSON.stringify(data, null, 2))
  try {
    await withRetry(() => dbx.filesUpload({ path, contents: buffer, mode: 'overwrite' }))
  } catch (err) {
    throw friendlyDropboxError(err)
  }
}
...
// Best-effort delete by the exact `pathname` uploadFile returned
// (path_display) — callers treat a failure here as non-fatal cleanup
// (replacing a file, or deleting its owning template), so a not-found path
// is a silent no-op rather than an error, same as readJsonFile's fallback
// contract.
export async function deleteFile(path) {
  if (!path) return
  const dbx = await getDbx()
  try {
    await withRetry(() => dbx.filesDeleteV2({ path }))
  } catch (err) {
