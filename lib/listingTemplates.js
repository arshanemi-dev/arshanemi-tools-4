...
export async function createTemplateMeta({
  ...
  marketplaceName, category1, category2, category3, category4, exportVersion, aiRules, rowCounts,
}) {
  const rows = await getTemplateRows()
  const meta = {
    id: nanoid(10),
    ...
    // and independent of templateName itself now that it's a real typed field.
    finalName: [marketplaceName, category1, exportVersion].map((s) => s?.trim()).filter(Boolean).join('_'),
